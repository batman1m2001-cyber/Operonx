...
from operon.core.media import extract_media
from operon.core.tracing.models import TraceNode, TraceSummary
from operon.core.utils.algo import build_children, tree_walk
from operon.core.utils.algo import topo_rank as compute_topo_rank
...
class TraceCollector:
    ...
    def _add_context_groups(
        ...
        # Per-call cache of labels drained from operon.core.tracing.labels.
        ...
        for trace_key, meta in node_meta.items():
            ...
            for i in range(len(relative)):
                ...
                if synthetic_key not in synthetics and synthetic_key not in node_lookup:
                    ...
                    # Check for a user-supplied label (via operon.core.tracing.label).
                    ...
                    if state_labels and spawner_key and spawner_key in node_lookup:
                        ...
                        if spawner_op:
                            ...
                            if cache_key not in label_cache:
                                from operon.core.tracing.labels import get_labels
