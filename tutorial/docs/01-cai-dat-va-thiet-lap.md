...
| `HUSH_TRACES_DB` | Tuỳ chọn | `~/.hush/ui-hush-eyes.db` | Đường dẫn SQLite lưu traces (ui-hush-eyes server) |
...
- **ui-hush-eyes server** — standalone Rust server với web UI (http://localhost:8420)
- **HushEyesTracer** — gửi traces từ engine đến ui-hush-eyes server
- **FlushWorker** — flush traces đến backends (ui-hush-eyes, Langfuse, OTEL) trong background threads
