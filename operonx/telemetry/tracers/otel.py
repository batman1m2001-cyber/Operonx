...
Inherits from operonx.core.tracing.Tracer. Flush runs in FlushWorker's
...
from operonx.telemetry.tracers._base import ConfigurableTracer

if TYPE_CHECKING:
    from operonx.core.tracing.trace_filter import TraceFilter
    from operonx.telemetry.backends.otel import OTELConfig
...
class OTELTracer(ConfigurableTracer):
    ...
    Example:
        ```python
        from operonx.telemetry import OTELTracer, OTELConfig
    ...
    def _make_client(self, config):
        from operonx.telemetry.backends.otel import OTELClient
