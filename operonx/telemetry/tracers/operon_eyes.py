"""OperonEyesTracer — sends traces to ui-operonx-eyes local server.
...
class OperonEyesTracer(Tracer):
    """Tracer that sends traces to the ui-operonx-eyes local server.

    ui-operonx-eyes is a lightweight Rust server that stores traces in SQLite
    ...
    def flush(self, trace_data: Dict[str, Any]) -> None:
        """POST trace data to ui-operonx-eyes /api/ingest endpoint.
        ...
        try:
            with urllib.request.urlopen(req, timeout=5) as resp:
                if resp.status != 200:
                    LOGGER.warning(
                        "ui-operonx-eyes returned status %d: %s",
                        resp.status,
                        resp.read().decode(),
                    )
        except Exception:
            LOGGER.debug(
                "Could not reach ui-operonx-eyes at %s (server may not be running)",
                self._url,
            )
