...
Usage (from inside an async-gen ``@op``):
    ...
    from operonx.core.tracing import label
...
``display_name`` for the ``stream_context`` wrapper that Operon would
...
``label()`` outside a generator op (or outside any Operon run) is a no-op.
"""
...
def label(name: str) -> None:
    ...
    outside any Operon run. Safe to call from anywhere.
