...
Requirements:
    hush-core and hush-runtime must be installed.
"""
...
from hush.core.ops.iteration import Each, ForOp, MapOp
...
# =============================================================================
# Pattern 5: MapOp parallel loop with nested graph body
# =============================================================================


def build_map_loop(n: int):
    """MapOp iterating over n items, each running inner_pipeline internally."""
    with GraphOp(name=f"map_loop_{n}") as g:
        with MapOp.of(
...
async def main():
    ...
    print("  Patterns: linear, nested, parallel, branching, MapOp, production, CPU-contention, CPU-production, CPU-chain")
    print("=" * 130)
    ...
    # --- MapOp loop ---
    print_header("MapOp parallel loop")
    for n in [10, 50, 100]:
        items = [f"item{i}" for i in range(n)]
        await bench_one(f"map_loop({n} items)", build_map_loop(n), {"items": items, "prefix": "test"})
