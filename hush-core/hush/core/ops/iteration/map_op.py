...
class MapOp(BaseIterationOp):
    ...
    async def _execute(
        ...
        if not iteration_data:
            LOGGER.warning(
                "[title]\\[%s][/title] MapOp [highlight]%s[/highlight]: no iteration data.",
                request_id,
                self.full_name,
            )
        ...
        if iteration_data and error_count / len(iteration_data) > 0.1:
            LOGGER.warning(
                "[title]\\[%s][/title] MapOp [highlight]%s[/highlight]: high error rate [muted](%s)[/muted].",
                request_id,
                self.full_name,
    ...
    @shorthand
    def of(cls, **kwargs) -> "MapOp":
        """Create a MapOp with flat kwargs.

        Example::

            with MapOp.of(x=Each([1, 2, 3]), multiplier=10, max_concurrency=4) as loop:
