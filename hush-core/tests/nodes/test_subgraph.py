"""Tests for @subgraph decorator."""

import pytest

from hush.core.engine import Hush
from hush.core.nodes.base import END, PARENT, START
from hush.core.nodes.graph.graph_node import GraphNode, subgraph
...
class TestSubgraphAutoName:
    """Test auto-naming through @subgraph decorator."""
    ...
    def test_returns_graph_node(self):
        @subgraph
        def my_flow(val):
            step = double(x=val)
            step["result"] >> PARENT["result"]
...
class TestSubgraphNested:
    """Test @subgraph inside another graph."""
    ...
    def test_subgraph_in_graph(self):
        ...
        assert isinstance(outer._nodes["sub"], GraphNode)
...
class TestSubgraphExecution:
    """Test full async execution of @subgraph."""
    ...
    async def test_with_renamed_outputs(self):
        """Explicit output renaming inside subgraph + explicit in outer graph."""
