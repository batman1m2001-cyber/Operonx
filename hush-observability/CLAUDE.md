...
**client.py:**
```python
from hush.observability.backends.mybackend.config import MyBackendConfig
...
def mybackend_plugin(config: dict):
    from hush.observability.backends.mybackend import MyBackendConfig, MyBackendClient
...
async def test_langfuse_tracer():
    with patch("hush.observability.tracers.langfuse.LangfuseClient") as mock_client:
