...
External tracing backend implementations for hush-icore. Mirrors Python's `hush-telemetry` package. All tracers implement `hush_icore::tracing::Tracer`.
...
```rust
use hush_telemetry::HushEyesTracer;
...
```rust
use hush_telemetry::langfuse::{LangfuseTracer, config::LangfuseConfig};
...
```rust
use hush_telemetry::otel::{OtelTracer, config::OtelConfig};
...
cargo check -p hush-telemetry

# With OTEL
cargo check -p hush-telemetry --features otel
