//! rush-plugin — Macro for building Hush workflow op plugins as cdylib crates.
//!
//! Provides the `rush_plugin!` macro that generates the C ABI exports
//! required for rush-serve to load your ops at runtime via `libloading`.
//!
//! # Usage
//!
//! ```rust,ignore
...
//! // [dependencies]
//! // serde_json = "1"
//! // rush-plugin = { path = "../../rust/rush-plugin" }
//!
//! mod math;
//! mod text;
//!
...
//!
//! - `rush_call(name, inputs_json) -> *mut c_char` — call a regular op
//! - `rush_call_generator(name, inputs_json) -> *mut c_char` — call a generator op
//! - `rush_free_str(ptr)` — free a string returned by the above
//!
...
#[macro_export]
macro_rules! rush_plugin {
    // With both call and call_generator
    (
        call: { $($name:literal => $func:path),* $(,)? },
        call_generator: { $($gen_name:literal => $gen_func:path),* $(,)? }
    ) => {
        $crate::_rush_plugin_impl!($($name => $func),*; $($gen_name => $gen_func),*);
    };
    // call only, no generators
    (
        call: { $($name:literal => $func:path),* $(,)? }
    ) => {
        $crate::_rush_plugin_impl!($($name => $func),*;);
    };
}

#[doc(hidden)]
#[macro_export]
macro_rules! _rush_plugin_impl {
    ($($name:literal => $func:path),*; $($gen_name:literal => $gen_func:path),*) => {
        ...
        /// Caller must free the returned pointer via `rush_free_str`.
        #[no_mangle]
        pub unsafe extern "C" fn rush_call(
        ...
        /// Same as `rush_call`.
        #[no_mangle]
        pub unsafe extern "C" fn rush_call_generator(
            name_ptr: *const std::ffi::c_char,
            input_ptr: *const std::ffi::c_char,
        ) -> *mut std::ffi::c_char {
            let name = match unsafe { std::ffi::CStr::from_ptr(name_ptr) }.to_str() {
        ...
        /// Free a string returned by `rush_call` or `rush_call_generator`.
        ///
        /// # Safety
        /// Pointer must have been returned by `rush_call` or `rush_call_generator`.
        #[no_mangle]
        pub unsafe extern "C" fn rush_free_str(ptr: *mut std::ffi::c_char) {
            if !ptr.is_null() {
