...
Chạy: cd examples && uv run python 01_hello_world/client.py
"""
...
def test_serve(label, script, port):
    ...
    try:
        # Warmup
        post(port, "/", {"name": "warmup"})
        # Measure N requests, report median
        times = []
        for _ in range(N_REQUESTS):
            result, elapsed = post(port, "/", {"name": "Hush"})
...
def main():
    ...
    print("=" * 55)
    test_serve("Python", "01_hello_world/serve_python.py", PORT_PY)

    print()
    print("=" * 55)
    print("2. Hush serve (Rust backend — Axum)")
    print("=" * 55)
    test_serve("Rust", "01_hello_world/serve_rust.py", PORT_RS)
