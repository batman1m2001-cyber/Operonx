...
Chạy: cd examples && uv run python 06_tracing/otel/client.py
"""
...
def test_serve(label, script, port):
    ...
    try:
        ...
        for _ in range(N_REQUESTS):
            result, elapsed = post(port, "/", {"text": "Hush workflow engine for GenAI"})
...
def main():
    ...
    print("=" * 55)
    test_serve("Python", "06_tracing/otel/serve_python.py", PORT_PY)

    print()
    print("=" * 55)
    print("2. Hush serve (Rust backend - Axum)")
    print("=" * 55)
    test_serve("Rust", "06_tracing/otel/serve_rust.py", PORT_RS)
