"""06 Tracing / OTEL — Test serve modes.

Chạy: cd examples && uv run python ex06_tracing/otel/bench.py
"""
...
ENDPOINTS = [
    ("/", {"text": "Hush workflow engine for GenAI"}, "Tracing OTEL"),
]
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            for path, payload, name in ENDPOINTS:
                ...
                report(times, errors)
                print("    Traces exported via OTLP.")
...
async def main():
    ...
    print("=" * 55)
    await test_serve("Python", "ex06_tracing/otel/serve_python.py", PORT_PY)

    print()
    print("=" * 55)
    print("2. Hush serve (Rust backend — Axum)")
    print("=" * 55)
    await test_serve("Rust", "ex06_tracing/otel/serve_rust.py", PORT_RS)
