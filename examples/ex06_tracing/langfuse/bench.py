"""06 Tracing / Langfuse — Test serve modes.

Chạy: cd examples && uv run python ex06_tracing/langfuse/bench.py
"""
...
ENDPOINTS = [
    ("/", {"text": "Hush workflow engine for GenAI"}, "Tracing Langfuse"),
]
...
async def main():
    ...
    print("=" * 55)
    await test_serve("Python", "ex06_tracing/langfuse/serve_python.py", PORT_PY)

    print()
    print("=" * 55)
    print("2. Hush serve (Rust backend — Axum)")
    print("=" * 55)
    await test_serve("Rust", "ex06_tracing/langfuse/serve_rust.py", PORT_RS)
