...
Chạy: cd examples && uv run python ex09_agent_workflow/bench.py
"""
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            await asyncio.gather(
                *[_warmup("/agent", {"query": "warmup"}) for _ in range(N_REQUESTS)]
            )
...
async def main():
    ...
    print("=" * 55)
    await test_serve("Python", "ex09_agent_workflow/serve_python.py", PORT_PY)

    print()
    print("=" * 55)
    print("2. Hush serve (Rust backend — Axum)")
    print("=" * 55)
    await test_serve("Rust", "ex09_agent_workflow/serve_rust.py", PORT_RS)
