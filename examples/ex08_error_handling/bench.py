"""08 Error Handling — Test serve modes: Python (FastAPI) and Rust (Axum).

Endpoints tested:
  POST /error-capture    — empty input, expects error in state
  POST /error-routing    — {a, b} input, expects output string
  POST /retry-fallback   — {query} input, expects output + used_fallback
  POST /llm-fallback     — {query} input, expects answer + model

Chạy: cd examples && uv run python ex08_error_handling/bench.py
"""
...
ENDPOINTS = [
    ...
    ("/error-routing", {"a": 10, "b": 3}, "Error Routing (success)"),
    ("/error-routing", {"a": 10, "b": 0}, "Error Routing (error)"),
    ("/retry-fallback", {"query": "What is AI?"}, "Retry + Fallback"),
]
...
async def main():
    ...
    print("=" * 55)
    await test_serve("Python", "ex08_error_handling/serve_python.py", PORT_PY)

    print()
    print("=" * 55)
    print("2. Hush serve (Rust backend — Axum)")
    print("=" * 55)
    await test_serve("Rust", "ex08_error_handling/serve_rust.py", PORT_RS)
