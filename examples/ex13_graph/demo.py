...
async def example_1_basic():
    ...
    result = await Hush(g, env=ENV_FILE, resources=RESOURCES_FILE).run(inputs={"input": 5})
...
async def example_2_chained():
    ...
    result = await Hush(g, env=ENV_FILE, resources=RESOURCES_FILE).run(inputs={"input": 3})
...
async def example_3_renamed_outputs():
    ...
    result = await Hush(g, env=ENV_FILE, resources=RESOURCES_FILE).run(inputs={"input": 7})
...
async def example_4_multi_params():
    ...
    result = await Hush(g, env=ENV_FILE, resources=RESOURCES_FILE).run(inputs={"x": 3, "y": 7})
    print("  (3 + 7) * 2 = 20")
