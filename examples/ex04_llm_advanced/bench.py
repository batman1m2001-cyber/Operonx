...
Chạy: cd examples && uv run python ex04_llm_advanced/bench.py
"""
...
async def test_serve(label, script, port):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            await asyncio.gather(*[_warmup("/", {"text": "warmup"}) for _ in range(N_REQUESTS)])
...
async def main():
    ...
    print("=" * 55)
    await test_serve("Python", "ex04_llm_advanced/serve_python.py", PORT_PY)

    print()
    print("=" * 55)
    print("2. Hush serve (Rust backend — Axum)")
    print("=" * 55)
    await test_serve("Rust", "ex04_llm_advanced/serve_rust.py", PORT_RS)
