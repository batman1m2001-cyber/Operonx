"""02 Data Pipeline — Test serve modes: Python (FastAPI) and Rust (Axum).

Endpoints tested:
  POST /data-pipeline   — fetch → transform → aggregate (no inputs)
  POST /text-pipeline    — clean → count_words → summarize (text input)

Chạy: cd examples && uv run python 02_data_pipeline/client.py
"""
...
ENDPOINTS = [
    ...
    ("/text-pipeline", {"text": "Tri tue nhan tao dang thay doi cach chung ta song"}, "Text Pipeline"),
]
...
def main():
    ...
    print("=" * 55)
    test_serve("Python", "02_data_pipeline/serve_python.py", PORT_PY)

    print()
    print("=" * 55)
    print("2. Hush serve (Rust backend — Axum)")
    print("=" * 55)
    test_serve("Rust", "02_data_pipeline/serve_rust.py", PORT_RS)
