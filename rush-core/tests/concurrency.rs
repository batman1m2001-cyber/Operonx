...
//!
//! Converted from test_concurrency.py. Uses Arc<Rush> + std::thread::spawn
//! since Rush::run_json(&self) creates a fresh EngineState per call.
...
fn test_concurrent_correctness_30_runs() {
    ...
    let engine = Arc::new(make_rush(config));
