# Rush-Core Big Update — Streaming Architecture Alignment

**STATUS: ALL 10 PHASES COMPLETE + rush-providers alignment** ✓

Last updated: 2026-03-10
Branch: `feat/stream-architecture`

## Post-Phase: rush-providers alignment ✓

Aligned rush-providers Rust ops with current hush-providers Python ops:
...
Tests: 122 rush-providers + all rush-core pass.
...
rush-core must be updated to match. Generator ops are critical — not optional.
...
| Provider streaming (LLM) | `async for chunk in stream:` | `rush-providers::execute_streaming()` already has `Sender<Value>` |
| Custom generator | User `yield` | Fn returns `Vec<Value>` or `impl Iterator` |

### Scheduler: sync batch-queue → async event-queue

Current rush-core scheduler is sync + rayon. Must become async + tokio event-queue
...
| Async public API | Wrap async scheduler in `block_on` for sync Rush::run_json |
