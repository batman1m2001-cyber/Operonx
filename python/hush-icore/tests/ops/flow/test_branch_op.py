"""Tests for BranchOp - conditional routing node."""

import pytest

from hush.core.ops.base import END, PARENT, START
from hush.core.ops.flow.branch_op import Branch, BranchOp, if_
...
class TestBasicScoreRouting:
    ...
    def score_graph(self):
        ...
        with GraphOp(name="score_workflow") as graph:
...
class TestMultipleVariablesRouting:
    ...
    def user_routing_graph(self):
        ...
        with GraphOp(name="user_workflow") as graph:
            user_data = FuncOp(
                name="user_data", code_fn=lambda: {"age": 25, "verified": True}, inputs={}
            )

            # Create actual target nodes
            adult_verified = FuncOp(
                name="adult_verified",
...
class TestBranchSchemaExtraction:
    ...
    def test_outputs_have_target_and_matched(self):
        ...
        branch = BranchOp(name="test", cases=[(PARENT["x"] > 0, "positive")], default="zero")
...
class TestBranchFluentBuilder:
    ...
    def test_basic_fluent_syntax(self):
        ...
        # Verify it's a BranchOp
        assert isinstance(branch, BranchOp)
    ...
    def test_fluent_with_node_targets(self):
        ...
        with GraphOp(name="fluent_graph") as graph:
            excellent = FuncOp(name="excellent", code_fn=lambda: {"grade": "A"}, inputs={})
            good = FuncOp(name="good", code_fn=lambda: {"grade": "B"}, inputs={})
            fail = FuncOp(name="fail", code_fn=lambda: {"grade": "F"}, inputs={})
    ...
    async def test_fluent_in_graph_execution(self):
        ...
        with GraphOp(name="fluent_workflow") as graph:
            branch = (
                Branch("scorer")
                .if_(PARENT["score"] >= 90, "excellent")
                .if_(PARENT["score"] >= 70, "good")
...
class TestIfShorthand:
    ...
    def test_if_else_basic(self):
        ...
        assert isinstance(router, BranchOp)
    ...
    async def test_if_in_graph_execution(self):
        ...
        with GraphOp(name="if_workflow") as graph:
...
class TestLocalVariablePattern:
    ...
    def test_local_variable_basic(self):
        ...
        mock_op = type("MockOp", (), {"full_name": "task_classifier"})()
        call_type = Ref(mock_op, "call_type")
    ...
    def test_multiple_local_variables(self):
        ...
        mock_op = type("MockOp", (), {"full_name": "classifier"})()
        call_type = Ref(mock_op, "call_type")
        is_active = Ref(mock_op, "is_active")
...
class TestCompoundBooleanInBranch:
    """Test compound boolean operations (& and |) in BranchOp."""

    def test_and_condition(self):
        """Test & condition in branch."""
        mock_op = type("MockOp", (), {"full_name": "data"})()
        score = Ref(mock_op, "score")
        status = Ref(mock_op, "status")
    ...
    def test_not_condition(self):
        ...
        mock_op = type("MockOp", (), {"full_name": "data"})()
        is_disabled = Ref(mock_op, "is_disabled")
    ...
    def test_complex_compound(self):
        ...
        mock_op = type("MockOp", (), {"full_name": "data"})()
        closed_by = Ref(mock_op, "closed_by")
        customer_silent = Ref(mock_op, "customer_silent")
        force_flag = Ref(mock_op, "force_flag")
    ...
    def test_get_all_vars_extracts_compound_vars(self):
        """Test that BranchOp extracts all vars from compound conditions."""
        mock_op = type("MockOp", (), {"full_name": "data"})()
        a = Ref(mock_op, "a")
        b = Ref(mock_op, "b")
        c = Ref(mock_op, "c")
