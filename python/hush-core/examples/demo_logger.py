...
def demo_all_project_logs():
    ...
    logger.error(
        "Error closing [highlight]InMemoryStreamService[/highlight]: %s", "Resources in use"
    )
    ...
    logger.error("Không thể khởi tạo global hub: FileNotFoundError")
    ...
    logger.warning("Config class không xác định: UnknownConfig")
    ...
    logger.debug("Đã đăng ký config class: OpenAIConfig")

    # resource_factory.py:67 - Factory handler registered (DEBUG)
    logger.debug("Đã đăng ký factory handler cho: LLMConfig")

    # resource_factory.py:131 - Resource creation error (ERROR)
    logger.error("Không thể tạo resource cho OpenAIConfig: API key not found")
    ...
    logger.error("File JSON không hợp lệ: Expecting property name")

    # yaml.py:77 - Invalid YAML (ERROR)
    logger.error("File YAML không hợp lệ: mapping values are not allowed here")
