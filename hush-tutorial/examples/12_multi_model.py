...
async def example_1_parallel_models():
    ...
    with GraphOp(name="multi-model-parallel") as graph:
        p = PromptOp.of(
...
async def example_2_cost_routing():
    ...
    with GraphOp(name="smart-routing") as graph:
...
async def example_3_load_balancing():
    ...
    with GraphOp(name="load-balanced") as graph:
        p = PromptOp.of(
...
async def example_4_fallback():
    """LLMOp fallback — tự động thử model tiếp theo khi primary fails."""
    print()
    print("=" * 50)
    ...
    with GraphOp(name="fallback-demo") as graph:
        p = PromptOp.of(
            template={"system": "Answer briefly.", "user": "{query}"},
            query=PARENT["query"],
        )
