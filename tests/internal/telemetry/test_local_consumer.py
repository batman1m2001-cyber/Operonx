...
def _mkexec(op_id, op_name, start, end, ctx=("main",),
            inputs=None, outputs=None, upstreams=None):
    return OpExecution(
        op_id=op_id,
        op_name=op_name,
...
class TestNodesJsonl:
    ...
    def test_media_offloaded_as_ref(self, tmp_path):
        ...
        node = _mkexec(
            "engine.stt#main", "stt", start=0.0, end=0.1,
            inputs={"audio": b"X" * 4096},
            outputs={"text": "hi"},
        )
        trace = WorkflowTrace(
            trace_id="t-media", workflow_name="w",
            started_at=0.0, ended_at=0.2, nodes=[node],
        )
