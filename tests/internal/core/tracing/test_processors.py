...
class TestDropOps:
    def test_drops_matching_full_name(self):
        events = [_ev(EventKind.OP_START, op_name="g.picker"),
...
class TestTruncateIO:
    ...
    def test_passes_short_values_unchanged(self):
        events = [_ev(EventKind.OP_START, op_name="g.a",
                      payload={"inputs": {"text": "short"}})]
        out = list(TruncateIO(max_bytes=2000)(events))
        assert out[0].payload["inputs"]["text"] == "short"

    def test_truncates_outputs_and_yielded(self):
        long = "y" * 3000
        events = [_ev(EventKind.OP_END, op_name="g.a",
                      payload={"outputs": {"r": long}})]
...
class TestRedactKeys:
    ...
    def test_custom_marker(self):
        events = [_ev(EventKind.OP_START, op_name="g.a",
                      payload={"inputs": {"api_key": "sk-secret"}})]
...
class TestGroupBy:
    def test_emits_start_and_end_around_first_group(self):
        ...
        assert kinds == [
            EventKind.GROUP_START, EventKind.OP_START,
            EventKind.OP_END, EventKind.GROUP_END,
        ]
    ...
    def test_multiple_groups(self):
        ...
        is_boundary = lambda e: e.kind is EventKind.OP_END and e.op_name == "g.a"
        out = list(GroupBy(boundary=is_boundary,
                           name_fn=lambda i: f"turn-{i}")(events))
...
class TestAggregate:
    def test_streams_running_tally_constant_memory(self):
        ...
        events = [
            ...
            _ev(EventKind.OP_YIELD, op_name="g.audio", seq=1,
                payload={"yielded": {"duration_ms": 20}, "idx": 0}),
            _ev(EventKind.OP_YIELD, op_name="g.audio", seq=2,
                payload={"yielded": {"duration_ms": 25}, "idx": 1}),
            _ev(EventKind.OP_YIELD, op_name="g.audio", seq=3,
    ...
    def test_passes_through_non_target_events(self):
        ...
        events = [
            ...
            _ev(EventKind.OP_YIELD, op_name="g.audio", seq=1,
                payload={"yielded": {}, "idx": 0}),
            _ev(EventKind.OP_YIELD, op_name="g.vad", seq=2,
    ...
    def test_passes_events_outside_window_through(self):
        ...
        events = [
            _ev(EventKind.OP_YIELD, op_name="g.audio", seq=0,
                payload={"yielded": {}, "idx": 0}),  # before any group
            _ev(EventKind.GROUP_START, seq=1, payload={"name": "t"}),
            _ev(EventKind.OP_YIELD, op_name="g.audio", seq=2,
                payload={"yielded": {}, "idx": 1}),  # inside — consumed
            _ev(EventKind.GROUP_END, seq=3, payload={"name": "t", "status": "ok"}),
        ]
        agg = Aggregate(
            op_name="g.audio",
        ...
        audio_yields = [e for e in out
                        if e.kind is EventKind.OP_YIELD and e.op_name == "g.audio"]
...
class TestProcessorComposition:
    def test_groupby_then_aggregate_replicates_turn_summary(self):
        ...
        events = [
            ...
            _ev(EventKind.OP_END, op_name="g.asr_result", seq=3,
                payload={"status": "ok"}),
        ]

        is_turn_boundary = lambda e: (e.kind is EventKind.OP_END
                                      and e.op_name == "g.asr_result")
        groupby = GroupBy(boundary=is_turn_boundary,
                          name_fn=lambda i: f"turn-{i}")
        aggregate = Aggregate(
            op_name="g.audio",
