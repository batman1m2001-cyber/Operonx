...
class TestLLMOpIntegration:
    ...
    async def test_llm_node_with_hub(self, hub):
        ...
        if not hub.has("llm:gpt-4o-mini"):
            pytest.skip("llm:gpt-4o-mini not configured in resources.yaml")

        node = LLMOp(name="chat", resource="gpt-4o-mini")
    ...
    async def test_llm_node_streaming_with_tokens(self, hub):
        ...
        node = LLMOp(name="stream_chat", resource="gpt-4o-mini", stream=True)
...
class TestLLMOpLoadBalancing:
    ...
    def test_load_balancing_init_with_list(self, hub):
        ...
        node = LLMOp(name="lb_test", resource=["gpt-4o", "gpt-4o-mini"], ratios=[0.7, 0.3])
    ...
    def test_load_balancing_default_ratios(self, hub):
        ...
        node = LLMOp(name="lb_default_test", resource=["gpt-4o", "gpt-4o-mini"])
    ...
    def test_select_llm_distribution(self, hub):
        ...
        node = LLMOp(name="dist_test", resource=["gpt-4o", "gpt-4o-mini"], ratios=[0.8, 0.2])
        ...
        claude_ratio = selections["gpt-4o-mini"] / 1000
    ...
    def test_load_balancing_metadata(self, hub):
        ...
        node = LLMOp(name="meta_test", resource=["gpt-4o", "gpt-4o-mini"], ratios=[0.6, 0.4])
    ...
    async def test_load_balancing_execution(self, hub):
        ...
        node = LLMOp(name="lb_exec_test", resource=["gpt-4o", "gpt-4o-mini"], ratios=[0.5, 0.5])
        node._ensure_initialized()
        ...
        async for _, result in node.run(state):
            pass
        assert "content" in result
        assert result["model_used"] in ["gpt-4o", "gpt-4o-mini"]
...
class TestLLMOpFallback:
    ...
    def test_fallback_init(self, hub):
        ...
        node = LLMOp(name="fallback_test", resource="gpt-4o", fallback=["gpt-4o-mini"])
        node._ensure_initialized()

        assert node.fallback == ["gpt-4o-mini"]
    ...
    def test_fallback_in_metadata(self, hub):
        ...
        node = LLMOp(name="meta_fallback_test", resource="gpt-4o", fallback=["gpt-4o-mini"])

        metadata = node.specific_metadata
        assert "fallback" in metadata
        assert metadata["fallback"] == ["gpt-4o-mini"]
    ...
    async def test_fallback_with_valid_primary(self, hub):
        ...
        node = LLMOp(name="fallback_exec_test", resource="gpt-4o", fallback=["gpt-4o-mini"])
...
class TestLLMOpTools:
    ...
    async def test_tools_function_calling(self, hub):
        ...
        node = LLMOp(
            name="tool_test",
            resource="gpt-4o",
            inputs={"prompt": None, "tools": None, "tool_choice": None},
        )
