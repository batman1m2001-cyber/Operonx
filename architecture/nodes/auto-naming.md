...
Khi tạo node mà không truyền `name=`, Hush tự động lấy tên biến từ assignment statement:
...
```
auto_name() called from BaseNode.__init__
    │
    ▼
...
_name_from_source(filename, lineno)  ← Fallback
    │ success? return name
    ▼ fail?
return None  → BaseNode dùng unique_name()
```
...
1. **`__init__` methods**: Constructor chain (`BaseNode.__init__` → `GraphNode.__init__` → ...)
2. **Registered code objects**: Functions đăng ký qua `register_skip()` — bao gồm `.of()` classmethods, `@code_node` wrapper, `@subgraph` wrapper
...
```python
# Source: llm = LLMNode.of(resource_key="gpt-4o")
#
# Bytecode:
#   LOAD_GLOBAL    LLMNode
#   LOAD_ATTR      of
...
|----------|-------------------|
| `@shorthand` decorator | `.of()` classmethods (ForLoopNode.of, MapNode.of, ...) |
| `@code_node` decorator | wrapper function |
| `@subgraph` decorator | wrapper function |
| `Branch.else_()` | method |
| `Branch.build()` | method |
...
```python
# Không skip: auto_name tìm thấy "wrapper" bên trong code_node wrapper
@code_node
def greet(name: str):
    return {"greeting": f"Hello, {name}!"}

g = greet(name="world")  # Muốn name == "g"
# Call stack: BaseNode.__init__ → wrapper → caller (g = ...)
#                                 ↑ skip này!
```

## Integration với BaseNode
...
| `nodes = [BaseNode()]` | `name == "nodes"` (bytecode sees list store) |
| `BaseNode()` (no assignment) | UUID fallback |
| `a = b = BaseNode()` | Bytecode thấy store đầu tiên |
| REPL / exec | Bytecode vẫn hoạt động |

## Tạo custom factory với auto-naming

Nếu bạn tạo factory function cho node:
...
node = my_factory(x=10)  # node.name == "node"
```

Xem thêm: [creating-custom-node.md](creating-custom-node.md) cho chi tiết về custom node patterns.
